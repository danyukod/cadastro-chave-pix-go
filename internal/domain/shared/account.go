...
func (a *accountDomain) Validate() error {
	...
	if a.accountType.EnumIndex() == 0 {
		businessErrors = AddError(businessErrors, *NewBusinessError("Account Type", "O tipo de conta esta invalido.", "accountType"))
	}
	if a.number <= 0 || a.number > 99999999 {
		businessErrors = AddError(businessErrors, *NewBusinessError("Account Number", "O numero da conta esta invalido.", "accountNumber"))
	}
	if a.agency <= 0 || a.agency > 9999 {
		businessErrors = AddError(businessErrors, *NewBusinessError("Agency Number", "O numero da agencia esta invalido.", "agencyNumber"))
	}
